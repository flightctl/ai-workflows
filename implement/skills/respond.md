...
```bash
python3 "$PR_COMMENTS_SCRIPT" reply --owner {owner} --repo {repo} --pr {pr-number} --body-file .artifacts/implement/{issue-key}/tmp-reply.md --comment-id {comment-id}
```
...
```bash
python3 "$PR_COMMENTS_SCRIPT" log --responses-log .artifacts/implement/{issue-key}/responses.jsonl --comment-id {id from fetch output} --response-summary "{brief summary of response}"
```
