...
- **Generalize customer-specific details.** When capturing Jira content into artifacts, replace customer names, organization names, hostnames, cluster topology, account identifiers, and other deployment-specific details with generic descriptions. Preserve what the feature must support, not who reported it or where they run it (see `guidelines.md`).
