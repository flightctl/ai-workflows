...
- A session that starts at `/respond`, `/revise`, or a manual-edit record — with no prior
  `/draft`, `/plan`, or `commit` snapshot — sets `origin_untracked: true` and renders a
  disclaimer line, since the document's structure was never verified against the template
  from origin. This is independent of `provenance_kind` (see below).
- `origin_untracked` reflects only the *local* session log, not absolute ground truth.
  ...
  `capture_event` only appends, so a later `/draft` or `/plan` run lands past `events[0]`
  and leaves the flag set. Recovery requires deleting the stale `provenance.json` before
  the next `/draft` or `/plan` run so the log restarts cleanly with `draft` or `plan` as
  its first event.
...
|------|------|--------------|
| `session` | Log has draft/plan/revise/respond/manual-edit events | Authored / Final / Phases lines |
...
unless the log's very first-ever event was `draft` or `plan`. This covers both a
session that starts at `/respond`, `/revise`, or a manual-edit record, and a log
that started as a `commit_only` snapshot and later picked up authoring events
(e.g. `commit` followed by `revise`) — in both cases the document's structure
was never checked against the template from origin.
...
`manual-edit` — no prior `/draft`, `/plan`, or commit snapshot):
